...
class Generator(object):
    ...
    def build_encoder_layers(self, inputs, filters, use_bn=True, name=None):
        with tf.variable_scope(name):
            x = tf.layers.conv2d(inputs, filters, kernel_size=4, strides=2, padding='SAME',
    ...
    def build_decoder_layers(self, inputs, filters, prob=0.0, activation=None, name=None):
        with tf.variable_scope(name):
            x = tf.layers.conv2d_transpose(inputs, filters, kernel_size=4, strides=2, padding='SAME',
                                           kernel_initializer=tf.random_normal_initializer(0.02))
            x = tf.layers.batch_normalization(x, momentum=0.1, epsilon=1e-5, training=self.is_training)
            if prob > 0.0: x = tf.layers.dropout(x, prob)
            if activation is not None: x = activation(x)
            return x

    def build_encoder(self, inputs, reuse=False, name='encoder'):
        with tf.variable_scope(name, reuse=reuse):
            ...
            for filter in config:
                ...
                use_bn = True if n > 0 else False
                encoder.append(self.build_encoder_layers(input, filter, use_bn, 'encoder_%d' % (n + 1)))
        return encoder

    def build_decoder(self, encoder, reuse=False, name='decoder'):
        with tf.variable_scope(name, reuse=reuse):
            ...
            for (filter, prob) in config:
                ...
                activation = tf.nn.sigmoid if m == len(config) - 1 else tf.nn.relu
                decoder.append(self.build_decoder_layers(input, filter, prob, activation, 'decoder_%d' % (m + 1)))
        return decoder

    def build_unet(self, inputs, reuse=False, name='generator'):
        with tf.name_scope(name):
            encoder = self.build_encoder(inputs, reuse)
            decoder = self.build_decoder(encoder, reuse)
        return decoder
